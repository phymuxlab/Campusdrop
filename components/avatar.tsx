...
export function Avatar({
  ...
  if (preset) {
    ...
    return <span className={`avatarWrap ${size} ${preset.className}`} role="img" aria-label={preset.label}><Icon size={size === 'sm' ? 16 : size === 'lg' ? 30 : 21} /></span>;
  }
  ...
  return <span className={`avatarWrap ${size} avatarDefault classic`} role="img" aria-label={`${name} default avatar`}><UserRound size={size === 'sm' ? 16 : size === 'lg' ? 30 : 21} /></span>;
}
